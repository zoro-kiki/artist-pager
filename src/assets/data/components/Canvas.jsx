...
const DEFAULT_FILTERS = { country: 'All', categories: [] }; // Changed to array for multi-select
...
const Canvas = () => {
  ...
  const activeFilterCount = useMemo(() =>
    Number(filters.country !== 'All') + (filters.categories?.length || 0),
    [filters]);
  ...
  const rowData = useMemo(() => {
    ...
    const filtered = profilesData.filter(p => {
      ...
      const matchCategory = !filters.categories || filters.categories.length === 0 || filters.categories.includes(p.category);
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeSidebar} className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[150]" />
            <motion.aside
              initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
              transition={{ type: 'spring', damping: 40, stiffness: 280 }}
              className="fixed top-0 left-0 h-full w-full sm:w-[320px] bg-[#0a0a0a] z-[160] flex flex-col border-r border-white/5"
            >
              <div className="flex items-center justify-between px-7 py-7 border-b border-white/5">
                <div>
                ...
                <button onClick={closeSidebar} className="w-8 h-8 flex items-center justify-center rounded-full border border-white/10 text-white/30 hover:text-white transition-all">
              ...
              <div className="flex-1 overflow-y-auto sidebar-scroll px-7 py-8">
                <div className="mb-10">
                  <p className="text-[8px] uppercase tracking-[0.45em] text-white/20 font-semibold mb-5">Category</p>
                  <div className="flex flex-col gap-2.5">
                    {CATEGORIES.map(cat => {
                      const active = tempFilters.categories.includes(cat);
                      return (
                        <label
                          key={cat}
                          className="flex items-center gap-4 cursor-pointer group py-3 px-4 transition-all duration-200 border border-white/5"
                          style={{ background: active ? 'rgba(212,175,55,0.05)' : 'transparent', borderColor: active ? '#d4af3744' : '' }}
                        >
                          <input
                            type="checkbox"
                          ...
                          />
                          <span className="text-[10px] uppercase tracking-[0.2em] font-medium" style={{ color: active ? '#d4af37' : 'rgba(255,255,255,0.35)' }}>
                            {cat}
                          </span>
                        </label>
                      );
                ...
                <div className="mb-6">
                  <p className="text-[8px] uppercase tracking-[0.45em] text-white/20 font-semibold mb-5">Region</p>
                  <div className="relative border border-white/10 bg-white/5">
                    <select
              ...
              <div className="px-7 py-6 flex gap-3 border-t border-white/5">
                <button onClick={clearTemp} className="flex-1 py-3.5 text-[9px] uppercase tracking-[0.2em] text-white/20 border border-white/10 hover:text-white">Clear</button>
                <button onClick={applyFilters} className="flex-[2] py-3.5 bg-white text-black text-[9px] font-black uppercase tracking-[0.22em] hover:bg-[#d4af37]">Apply</button>
              </div>
            </motion.aside>
          </>
        )}
      ...
      <section ref={gridRef} className="absolute top-0 left-0 flex flex-col gap-8 md:gap-12 p-10 md:p-20 pt-32 md:pt-64 w-max will-change-transform">
        {rowData.rows.map((row, rowIndex) => (
          <div key={rowIndex} className="flex gap-8 md:gap-12" style={{ paddingLeft: rowIndex % 2 !== 0 ? '44px' : '0' }}>
            {row.map((profile, i) => (
              <LuxuryCard
